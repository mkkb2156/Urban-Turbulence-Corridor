...
### 座標系
- 內部一律 EPSG:3826（TWD97/TM2）→ API 輸出一律 EPSG:4326（WGS84）

### 資料來源（完整清單）

#### 風場數據
| 來源 | 解析度 | 免費 | 取得方式 | 優先度 | 用途 |
|------|--------|------|----------|--------|------|
| **Open-Meteo** | 9–25 km，4 高度 | ✅ | api.open-meteo.com | 🔴 關鍵 | 即時 + 72h 預報風場，逐時風速/風向/陣風 |
| **CWA 測站** | 點位，10 分鐘 | ✅ | opendata.cwa.gov.tw | 🔴 關鍵 | 本地觀測風速，地面真值校驗 |
| **CWA WRF** | 3 km | ✅ | opendata.cwa.gov.tw | 🔴 關鍵 | 高解析度數值預報，台灣本地模式 |
| **IGRA/板橋探空** | 點位，2 次/日 | ✅ | ncei.noaa.gov | 🟡 高 | 垂直風速剖面驗證，對數風速模型校正 |
| **ERA5** | 31 km，逐時 | ✅ | cds.climate.copernicus.eu | 🟡 高 | 長期氣候學統計，風玫瑰歷史基線 |
| **Global Wind Atlas** | 250 m | ✅ | globalwindatlas.info | 🟠 中 | 年均風速空間分佈，粗估風場背景值 |
| **MOENV 空品站** | 點位，逐時 | ✅ | data.moenv.gov.tw | 🟠 中 | 補充地面風速觀測，填補 CWA 測站空隙 |
| **民生公共物聯網** | 點位，不等 | ✅ | ci.taiwan.gov.tw | 🔵 低 | 微氣象站風速，密度高但品質不均 |

#### 地形數據
| 來源 | 解析度 | 免費 | 取得方式 | 優先度 | 用途 |
|------|--------|------|----------|--------|------|
| **台灣 20m DEM** | 20 m | ✅ | data.gov.tw/35430 | 🟡 高 | 高精度地形，台灣本地 DTM/DSM |
| **Copernicus DEM** | 30 m | ✅ | AWS S3（免認證） | 🔴 關鍵 | 全球 DSM 備援，快速取得無需申請 |
| **FABDEM** | 30 m DTM | ✅* | data.bris.ac.uk | 🟡 高 | 去除建築/樹冠的裸地形，z₀/zd 計算更準 |

#### 建築數據
| 來源 | 解析度 | 免費 | 取得方式 | 優先度 | 用途 |
|------|--------|------|----------|--------|------|
| **OSM（台北）** | 個體建築 | ✅ | osmnx / Geofabrik | 🔴 關鍵 | 建築足跡 + 部分高度，FAI/BCR 計算 |
| **GHS-BUILT-H** | 100 m 網格 | ✅ | GHSL portal / GEE | 🔴 關鍵 | 全球建築高度估計，填補 OSM 缺失高度 |
| **GHS-OBAT** | 個體建築 | ✅ | GEE community | 🟡 高 | 個體建築高度推估，精度優於網格 |
| **台北 LOD1 3D** | 個體建築 | ✅ | 3d.taipei（需申請） | 🔴 關鍵 | 台北市精確 3D 建築模型，最高品質 |

#### 土地覆蓋 / 植被
| 來源 | 解析度 | 免費 | 取得方式 | 優先度 | 用途 |
|------|--------|------|----------|--------|------|
| **ESA WorldCover** | 10 m | ✅ | worldcover2021.esa.int | 🟡 高 | 土地覆蓋分類，區分建成區/綠地/水體 |
| **Meta/WRI Canopy** | 1 m | ✅ | AWS S3 / GEE | 🟡 高 | 樹冠高度，影響粗糙度 z₀ 和風場遮蔽 |

#### 航空 / 無人機管制
| 來源 | 解析度 | 免費 | 取得方式 | 優先度 | 用途 |
|------|--------|------|----------|--------|------|
| **CAA 無人機空域** | 向量 | ✅ | drone.caa.gov.tw | 🔴 關鍵 | 禁飛區 / 限飛區圖層，合規性檢查 |
| **OpenSky Network** | 5 秒 | ✅** | opensky-network.org | 🔵 低 | 即時航機位置，避免有人機衝突參考 |

> ✅* FABDEM 需學術註冊下載
> ✅** OpenSky 免費版有 API 頻率限制
